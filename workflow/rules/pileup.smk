...
rule all_pileups:
    input:
        expand("mpileup/{sample}.mpileup.gz", sample=config["global"]["sample-names"])
        if "individual-samples" in config["settings"]["pileups"]
        else [],
        ("mpileup/all-samples.mpileup.gz" if "all-samples" in config["settings"]["pileups"] else []),
        (
            "mpileup/all-merged-samples.mpileup.gz"
